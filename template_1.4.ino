...
WiFiUDP udp;
const int localPort = 12002;  // Port to listen on for incoming messages
...
// Function to display WiFi connection time
void displayConnectionTime() {{
  AtomS3.Display.setTextSize(0.5);
  AtomS3.Display.setTextColor(TFT_WHITE);
  AtomS3.Display.drawString("Time: " + String(elapsedTime / 1000.0, 1) + "s", 
...
void displayWiFiStatus(wl_status_t status) {{
  ...
  AtomS3.Display.drawString(statusMsg, 
                           AtomS3.Display.width() / 2, 
                           AtomS3.Display.height() / 3);
...
void setup() {{
  ...
  AtomS3.Display.drawString("Powered by", AtomS3.Display.width() / 2, AtomS3.Display.height() / 1.7);
  AtomS3.Display.drawString("&", AtomS3.Display.width() / 2, AtomS3.Display.height() / 1.3);

  AtomS3.Display.setTextSize(0.7);
  AtomS3.Display.drawString("edybalasa", AtomS3.Display.width() / 2, AtomS3.Display.height() / 1.5);
  ...
  if (WiFi.status() == WL_CONNECTED) {{
    ...
    AtomS3.Display.setTextSize(0.5);
    AtomS3.Display.drawString("IP: " + WiFi.localIP().toString(), 
                             AtomS3.Display.width() / 2, 
...
void loop() {{
  ...
  // Check for button press to send camera number via port 12002
  if (millis() - lastButtonCheck > buttonCheckInterval) {{
    lastButtonCheck = millis();
    
    if (AtomS3.BtnA.wasPressed() && WiFi.status() == WL_CONNECTED) {{
      // Send camera number via port 12002
      byte cameraNumber = (byte)deviceId;
      udp.beginPacket(broadcastIP, 12002);
      udp.write(cameraNumber);
      udp.endPacket();
      
      Serial.printf("Button pressed, sent camera number %d via port 12002\n", deviceId);
    }}
  }}
